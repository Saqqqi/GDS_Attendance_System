...
const Break = ({ attendanceId: initialAttendanceId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!attendanceId) {
      alert("Attendance ID is missing. Please log in again.");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Failed to ${isBreakActive ? "end" : "start"} break: ${result.error}`);
      }
    } catch (error) {
      alert(`An error occurred while ${isBreakActive ? "ending" : "starting"} the break.`);
    }
  };
